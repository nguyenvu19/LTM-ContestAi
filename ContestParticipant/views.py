...
from ContestAdmin.models import Status, Contest, RegisterContest, Language
    ...
    def get(self, request):
        if request.user.is_active:
            ...
            try:
                ...
                id = userId
                allContest = Contest.objects.all().order_by('-id')
                # print(allContest)

                idContestRegis = RegisterContest.objects.filter(IDUser=request.user.id).values_list(
                    'IDContest', flat=True).order_by('-id')  # contest that user regis before
                ...
                allStatus = Status.objects.filter(
                    IDUser=request.user.id).order_by('-id')
                idContestSubmitted = allStatus.values_list(
                    'IDcontest', flat=True)
                statusContestSubmitted = allStatus.values_list(
                    'Status', flat=True)
                ...
                for id in list(idContestSubmitted):
                    ...
                    repeated = list(filter(lambda x: int(
                        x.id) == int(id), contestSubmitted))
                    if not repeated:
    ...
    def get(self, request, id):
        if request.user.is_active:
            ...
            try:
                ...
                selectedContest = Contest.objects.get(id=id)
                time_reg_string = selectedContest.TimeRegister.strftime(
                    "%Y-%m-%d %H:%M:%S")
                time_start_string = selectedContest.TimeStart.strftime(
                    "%Y-%m-%d %H:%M:%S")
                time_now = datetime.now()

                time_reg = datetime.strptime(
                    time_reg_string, "%Y-%m-%d %H:%M:%S")
                time_start = datetime.strptime(
                    time_start_string, "%Y-%m-%d %H:%M:%S")

                status = ''
                if time_now > time_reg and time_now < time_start:
                    try:
                        regisTemplate = RegisterContest(
                            IDContest=id, IDUser=userId)
                        regisTemplate.save()
                        status = 'OK'
                    except Exception as e:
                        print(e)
...
class Starting(View):
    def get(self, request, id):
        ...
        if request.user.is_active:
            ...
            try:
                ...
                time_start_string = selectedContest.TimeStart.strftime(
                    "%Y-%m-%d %H:%M:%S")
                time_end_string = selectedContest.TimeEnd.strftime(
                    "%Y-%m-%d %H:%M:%S")
                time_now = datetime.now()
                print(time_now)
    ...
    def post(self, request, id):
        ...
        if ".py" in name:
            ...
            t = Contest.objects.get(id=id).TimeOut * 8
            # print(t)
...
class Standing(View):
    def get(self, request, id):
        ...
        status = Status.objects.filter(
            IDcontest=id).order_by('-Status', 'TimeSubmit')
        timestart = Contest.objects.get(id=id).TimeStart
        data = []
        error = []
        for x in status:
